...
const WordCloud = ({ contentData }) => {
  ...
  return (
    <div>
      <ReactWordcloud
        words={getOccurencesFromTweets(contentData)}
        callbacks={callbacks}
        options={{ fontSizes: [20, 50] }}
      />
    </div>
  )
}
...
function getOccurencesFromTweets(contentData) {
  ...
  contentData.forEach((content) =>
    getFormattedWordsList(content.trim()).forEach((word) =>
      wordsOccurences.set(word, 1 + (wordsOccurences.get(word) || 0))
    )
  )
