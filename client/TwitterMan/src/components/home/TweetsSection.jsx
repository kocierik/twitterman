...
const TweetsSection = ({ tweetsDataFiltered, loadMore, rfp, setRfp }) => {
	...
	const currentTweets = tweetsDataFiltered.slice(rfp * (currentPage - 1), rfp * currentPage)
	...
	const getPageNumberItems = () => {
		let items = []
		for (let page = currentPage - 1 || 1; page <= currentPage + 1 || page <= 3; page++) {
			items.push(
				<li
					class={
						page === currentPage ?
	...
	return (
		<div>
			<div className="flex justify-end align-baseline m-5">
				<nav class="mr-2 border border-slate-500 rounded-lg " aria-label="Page navigation">
					<ul class="flex list-style-none">
						<li><a
							class={`relative block py-1.5 px-3 rounded border-0 bg-transparent outline-none transition-all duration-300 rounded ${currentPage === 1 ? "pointer-events-none text-gray-500" : " text-slate-200"} hover:text-gray-800 hover:bg-gray-200 focus:shadow-none`}
							onClick={() => changePage(currentPage - 1)}>Previous</a></li>
						{getPageNumberItems()}
						<li><a
							...
							onClick={() => changePage(currentPage + 1)}>Next</a></li>
					</ul>
				</nav>
				<select id="rfp" onChange={changeRfp} class="w-12 max-w-xs text-sm text-white text-center rounded-lg bg-gray-500/60 focus:border-blue-600 focus:outline-none">
