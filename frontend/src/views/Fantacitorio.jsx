...
function Fantacitorio() {
  ...
  const squadreFiltered = username
    ? squadre.filter((tweet) =>
        tweet.name.toLowerCase().startsWith(username.toLowerCase())
      )
    : squadre
  ...
  async function trovaSquadre() {
    ...
    for (let t of tw) {
      if (
        t.media != null &&
        t.media[0] != null &&
        t.media[0].width == 1024 &&
        t.media[0].height == 512
      ) {
        final.push(t)
      }
    }
      ...
      <div className="flex flex-col justify-center text-white">
        <div className="text-center">
          <div className="text-5xl text-center pt-10">
            Classifica <b>Fantacitorio</b>
          </div>
          <div className="text-3xl text-center mt-2 pb-5">
            <i>dell'ultima settimana</i>
          </div>
        </div>
        <div className="text-center flex justify-center p-5">
          <table
            class="text-sm text-left text-gray-500 dark:text-gray-300"
            width="700"
          >
            ...
            <tbody>
              {punteggi.map((p) => {
                return (
                  <tr
                    key={p.position}
                    class="bg-white border-b bg-white bg-slate-800 dark:border-gray-700"
                  >
                    <th
                      scope="row"
                      class="py-4 px-6 font-medium text-gray whitespace-nowrap dark:text-white"
                    >
                      {p.position}
                    </th>
        ...
        <div
          className={`box-border m-auto max-w-[75rem] 3xl:max-w-[120rem] columns-1xs ${
            squadreFiltered.length !== 1 ? 'sm:columns-2xs md:columns-2' : ''
          }`}
        >
